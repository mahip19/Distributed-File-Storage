...
\begin{itemize}
    ...
    \item \textbf{Storage}: In-memory \texttt{ConcurrentHashMap} used to simulate storage nodes (as per project constraints to demonstrate volatile storage handling).
\end{itemize}
...
\subsection{Not Achieved / Future Work}
\begin{itemize}
    \item \textbf{Dynamic Node Recovery}: While the system handles node failure, automatically re-joining a failed node and backfilling data (hinted handoff) is partially implemented but not fully automated.
...
make test
    ...
    \item \textbf{(Optional) Manual Execution on Multiple Machines}:
    If you wish to run nodes on \textit{separate} physical machines (e.g., linux-079, linux-080):
    \begin{itemize}
        \item Edit \texttt{nodes.conf} to replace \texttt{localhost} with the actual IP addresses (use \texttt{hostname -I} to find them).
        \item SSH into each machine and start the components manually:
        \begin{verbatim}
# On Machine 1 (Storage Node 1)
java -cp out com.distributed.storage.storage.StorageNode 8081

# On Machine 2 (Metadata Node 1)
java -cp out com.distributed.storage.metadata.MetadataNode 9091
        \end{verbatim}
    \end{itemize}
\end{enumerate}
